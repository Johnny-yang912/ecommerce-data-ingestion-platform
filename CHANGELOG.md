...
- **Endpoints changed from `async def` to `def`: synchronous DB calls no longer hold the event loop.** All three handlers (`/orders`, `/process_raw`, `/raw`) made blocking psycopg2 calls with no `await` anywhere in the connection-holding window — and `statement_timeout` is 30 seconds, so **a single stuck query froze an entire uvicorn process for 30 seconds**, not just that request. Measured: `/health`, which **touches no database at all**, went from p99 **167ms → 34ms** under load; throughput +42%, and the worker curve no longer inverts at 8 (207 → 485 RPS). **The fix is for failure amplification; the performance change is a side effect.** → [measured](./docs/en/verification/2026-09-02-sync-handlers-before-after.md)
