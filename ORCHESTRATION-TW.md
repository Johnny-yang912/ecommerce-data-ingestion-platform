...
本專案不寫規則（見上表）。但下面幾條是**訂規則時該遵守的原則**，
它們與流量大小無關，真實部署時直接適用。

**① 存活型與值型的門檻來源不同，不該混為一談**

值型門檻（延遲、錯誤率）**必須從觀測推導**——沒有真實流量的分佈，任何數字都是猜的。
存活型門檻（「這個來源多久沒說話算異常」）**來自排程或契約的宣告**：批次的窗口寫在
它的 cron 裡，持續攝入的窗口寫在上游的 SLA 裡。把「等真實流量」這個理由套到存活型
規則上，是過度套用。

**② 「沒有資料」不會觸發任何條件** ⭐

一般規則問「值有沒有超過門檻」，而資料不存在時查詢回傳空集合，
**空集合不符合任何條件，規則會安靜地待在 Normal**。
一條天真的「延遲 > 500ms」規則，在整個系統死掉時反而不會響。
故存活必須是**獨立的一條規則**，不能指望值型規則順帶抓到。

**③ ⚠️ cumulative temporality 會讓 `absent()` 抓錯東西**

OTel SDK 在 cumulative 模式下按固定間隔無條件送出累計值，**即使期間沒有任何事件**。
因此只要行程活著，序列就永遠存在、永遠不 absent。兩種故障要分成兩條規則：

| 問題 | 寫法 | 抓到的是 |
|---|---|---|
| 來源不再說話 | `absent(...)`，或 Grafana 的 No Data 狀態設定 | 行程死了、collector 死了、機器關機、斷網 |
| 來源活著但沒事件 | `increase(<計數器>[窗口]) == 0` | 上游停了——系統全綠、容器全 Up、零資料 |

**後者才是管線最需要的，而它恰好是 `absent()` 抓不到的那一個。**
選錯的代價不是誤報，是一條看起來很合理、卻永遠不會響的規則。

**④ 靜音時段只抑制通知，不抑制狀態轉換**

若用「短窗口 + 靜音預期空窗（夜間、維護窗）」而非「窗口長到涵蓋空窗」，要注意一條
在靜音期間已進入 Firing 的規則，**會在靜音解除的瞬間立刻發出**。靜音區間必須延伸到
「預期的第一批資料實際落地之後」，而不是空窗名義上的結束時間。

**⑤ 存活規則必須寫在被監控系統之外**

它要偵測的正是「我這側已經沒辦法說話了」（同 §2.12 ④）。這也是它與 §2.13 失敗通知的
分工：後者跑在 Airflow 的 task 行程裡，機器死了一起死；前者跑在雲端，機器整台關掉
照樣發得出來。兩者不是替代關係，而是**分屬不同故障域的兩條獨立路徑**——任何一條單獨
存在都有結構性盲區。

**⑥ 告警內容要帶處置，不只是哪個數字不對**

同 §2.13 ① 的理由：收到告警的人需要知道「先看哪裡」，而那個資訊不會自己出現在
指標名稱裡。
