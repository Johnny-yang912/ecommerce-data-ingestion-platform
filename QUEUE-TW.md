...
**熔斷器需要「先撞牆幾次」才會啟動**，這是它的固有性質，有兩個面向：

*高併發*：第一波之所以仍慢（18.4s），是因為 48 筆同時到達時三次失敗都還沒**返回**，
整波都已經越過閘門。成本上界是「每行程一個併發波次」，而非每筆請求；持續流量下這
一波只佔事故開頭的數秒。

*低流量*：每個行程要連續失敗 `ENQUEUE_BREAKER_FAILURE_THRESHOLD`（3）次才開路，而
請求是輪流分到各個 uvicorn 行程的。所以整個叢集要全部進入保護狀態，大約需要
**行程數 × 門檻** 筆失敗請求——4 個行程就是 12 筆。實測：broker 停機時循序送 6 筆，
每筆耗時 7～18s，log 顯示 4 個行程只有 **2 個** 觸發 `circuit_opened`，其餘從頭到尾
沒開過路，使用者吃到的是未受保護的完整逾時。

**低流量下不啟動是可以接受的，因為那時候並沒有東西需要被保護。** 熔斷器要防的是
§2.2 那種超線性崩潰，而崩潰是**併發現象**：6 筆循序請求不會互相排隊、不會耗盡連線
池、不會讓客戶端集體逾時，只是 6 筆慢請求。要避免的災難不存在時，不介入才是對的。

**刻意不調低門檻**：設成 1 會讓一次瞬斷就開路 30 秒，期間所有記錄改走恢復掃描
（最多延遲一個掃描間隔）。用「確實壞了」的證據換取誤判空間，比反過來划算。
