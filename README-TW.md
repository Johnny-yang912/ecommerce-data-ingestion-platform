...
一條電商訂單的攝入與分析管線，圍繞**資料生命週期管理**組織。不可信的入站資料透過分層品質契約逐步轉為可信的分析資料，而**每一次品質判斷的演進都保持可稽核**。

資料工程優先、後端工程其次：

- 攝入層的容錯確保**資料進得來**——多點重試、CAS 認領、崩潰恢復、有斷路器的派工。
- 分層品質契約確保**資料流得正確**——Raw → ODS → `stg_` → `int_` → `dim_`／`fct_` → `rpt_`，恰好阻斷一次。
- 規則版本化與 append-only 事件日誌確保**一個判斷可以被修訂，而不必改寫歷史**。
