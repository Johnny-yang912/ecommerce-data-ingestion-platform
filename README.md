...
- [ ] OpenTelemetry — remaining work
  - **Business / DQ metrics**: deliberately deferred. `seed_demo_daily` picks its dirty rate deterministically from five choices per day → **constant within a day**, so a minute-level error rate says nothing `rpt_quality_events_daily` does not already say; high-cardinality slicing belongs in the warehouse by definition (see the tier-1/tier-2 boundary in [DQ_ARCHITECTURE.md](./DQ_ARCHITECTURE.md))
  - **Airflow integration**: the Collector is ready (plaintext to localhost suffices); Airflow's built-in OTel exporter still needs its auth capability verified
  - **Absent alerting and dashboards**: for a **continuous** signal the threshold must be derived from observation, and this machine is powered off overnight — a rule written today would cry wolf every night. Needs 2–3 days of real cadence first. ⚠️ That argument does **not** extend to the **scheduled batches**: `orders_analytics_daily`'s window comes straight from its cron expression, and a laptop that is off at 22:30 is a true positive for a daily batch, not a false alarm. Failure notification (Phase 5 above) now covers "ran and failed"; "should have run, didn't" is the gap these rules exist to close
