...
⚠️ **But `raw_id`'s uniqueness only holds within a single landing instance.** Two independent ODS instances each start numbering at 1, so extracting both into one BQ staging table makes `raw_id`-grained dedup collapse unrelated orders into "copies" of each other — no error, no trace. That welds "one staging table maps to exactly one ODS" into the pipeline as an implicit premise; if multi-instance upstreams ever get their own landing layers, the dedup key must become something like `(source_instance, raw_id)`. Concrete instance in [ORCHESTRATION §5.4](./ORCHESTRATION.md).
...
Every push to `main` and every Pull Request automatically triggers GitHub Actions (`.github/workflows/ci.yml`), which installs dependencies and runs the full test suite (393 tests, 100% coverage of the gated modules) across a **Python 3.10 and 3.12** matrix.

A **separate DAG workflow** (`.github/workflows/dags.yml`, 52 tests) installs Airflow under the official constraints and parses `orchestration/dags/` with DagBag. It is deliberately not folded into the main job — Airflow's install is heavy and pins many package versions, which would destroy the main job's "mock DB, done in seconds" property. It needs no `DB_URL`, because DAG files deliberately import no project module (see [ORCHESTRATION §2.2](./ORCHESTRATION.md)).
...
├── check_raw_pending.py   # Dispatch-liveness probe (raw.status='pending' age; threshold derived
│                          #   from the recovery path's own settings). Run by raw_pending_watch
...
| [Orchestration Layer Architecture](./ORCHESTRATION.md) | Airflow's six DAGs and their decisions: the Airflow-is-not-a-task-queue boundary, why DAG files must not import project modules, one extract task per table, layered dbt execution with `--indirect-selection=buildable`, the structural reason for `catchup=False`, asymmetric retries, why observation signals (freshness and the dispatch probe) each get their own DAG, and the manual-trigger semantics of Proposal B; includes the runbook and the full Proposal B demo script |
...
- `rpt_*`: Pre-aggregated reporting tables built on top of dim/fct; fixed grain, optimised for dashboard performance and BigQuery query cost. Spans **two data domains**: business reports always read `dim_`/`fct_` (one definition per metric; never bypassing Gold's semantic decisions and tests); quality reports read `int_orders_quarantine` and `stg_quality_events` — quarantined rows by definition never reach Gold. Ratios are never materialized and `COUNT(DISTINCT)` is flagged non-additive; rationale in [ecommerce_dbt/README §7](./ecommerce_dbt/README.md)
...
- [v] dbt Core: dim_*/fct_* (Kimball header/line dual fact tables + two SCD1 dimensions — see [ecommerce_dbt/README](./ecommerce_dbt/README.md) §6)
- [v] dbt Core: rpt_* (three tables: `rpt_quality_events_daily` event-axis incremental, `rpt_quality_backlog` snapshot, `rpt_sales_daily_by_category` business aggregate; see [DQ_ARCHITECTURE.md](./DQ_ARCHITECTURE.md) and [ecommerce_dbt/README §7](./ecommerce_dbt/README.md)). ⚠️ Proposal B's event producer is unimplemented, so reinstatement-related columns are currently always 0. Scenario-specific `int_orders_*`, SCD2 `dim_customer`, making `rpt_sales_*` incremental, and monetary-exposure measures for quality reporting are all designed but each enabled only when its trigger fires
- [ ] Looker Studio connected to BigQuery dim_*/fct_*/rpt_*

**Phase 5 — Automation + Queue Upgrade**
- [v] Airflow (local, 3.0.0 + LocalExecutor), six DAGs, every schedule declared explicitly in `Asia/Taipei` — `seed_demo_daily` (the simulated upstream = this system's only data source; 10/13/17/21, 800 orders/day), `raw_pending_watch` (dispatch-liveness probe 30 min after each seeding slot; its threshold is derived from the recovery path's own settings rather than hardcoded), `orders_analytics_daily` (22:30; 2 extract tasks → 4 layered `dbt build`s → a full `dbt test`), `source_freshness_watch` (08:00; the backstop for extract), `dq_reevaluation` (Proposal B: manual trigger, dry-run by default, chains into the main DAG on commit), and `seed_demo_gate_demo` (the Hard Gate interception scenario, manual). **None of the four scheduled DAGs depends on another at the Airflow level — the ordering contract exists only in the time gaps** — and each one's red means a different response, which is the entire reason they are separate. dbt and the analytics scripts live in separate venvs; DAG files import no project module and are therefore CI-covered. Decisions in [ORCHESTRATION.md](./ORCHESTRATION.md)
- [v] Failure notification (`orchestration/dags/_notify.py`, 2026-08-24) — the four scheduled DAGs carry an `on_failure_callback` whose message states the **response**, not the task name ("the analytics pipeline is broken; tomorrow morning's report is stuck on yesterday"): that information previously lived only in each DAG file's docstring, and the person handling the incident does not read docstrings. Attached at task level rather than DAG level, because downstream `upstream_failed` tasks do not fire the callback → a broken seven-task chain sends exactly one message, and that message names the task that actually broke. ⚠️ **The transport defaults to a log line; a real channel is one `NOTIFY_WEBHOOK_URL` away** — deliberately not a notifier pointed at a connection that doesn't exist, because that behaves as "red → callback fires → it raises → nobody receives anything", and **believing you have alerting when you don't is far more dangerous than plainly having none**; every failure message therefore carries `channel=`, so `channel=log` tells you nobody was notified. **Covers "ran and failed" only**: should-have-run-didn't (Airflow 3 removed SLAs), machine powered off, and freshness's warn (exit 0, task green) are all invisible to it — those three need absent alerting. See [ORCHESTRATION §2.13](./ORCHESTRATION.md)
...
- [v] Dispatch circuit breaker (`circuit_breaker.py`) — degradation with the broker unavailable is **super-linear** (kombu's producer pool is capped at 10 per process): measured, 47 of 48 concurrent requests failed to complete within 120s, while their Raw records **had in fact been written** — the client saw only a timeout and resent. Three consecutive failures open the circuit and Redis is no longer touched: p50 drops from "timeout" to **5ms**. Also fixes the transaction opened by `db.refresh()` spanning the dispatch (measured: 23 of 32 pool slots stuck `idle in transaction` at 60 concurrent)
- [v] A bounded recovery scan — the circuit breaker keeps ingestion at full speed during an incident, so `pending` accumulates at full speed too; "select every pending row and dispatch one by one" simply relocated the collapse onto the recovery path. Now paginated with `LIMIT` + an `id` cursor (dispatching does not change `status`, so `LIMIT` alone re-fetches the same leading rows forever), capped pages per run, a Redis lock against overlap, and a grace period leaving freshly ingested rows to the ingestion path. Measured against a 120,000 backlog: scan #1 dispatched 100,000 (the cap), #2 dispatched 20,000, ODS grew by exactly 120,000 with zero duplicates. ⚠️ **A prerequisite is still open**: `raw.status` has no index, so pagination bounds memory and dispatch volume but not query cost; the index's shape must be decided from real traffic rather than from `EXPLAIN` over self-generated data (see [QUEUE.md §6.1](./QUEUE.md))
- [v] Docker extension: Redis + Celery Worker + Beat; the api runs 4 uvicorn workers with rate-limit counters in Redis
- [v] OpenTelemetry — traces + operational metrics (2026-08-17)
  - **Resident Collector** (contrib 0.158.0, `otel/collector-config.yaml`): apps always export to the local Collector; the cloud endpoint and credentials exist in exactly one place. ⚠️ `.env` deliberately **avoids** the OTel standard name `OTEL_EXPORTER_OTLP_ENDPOINT` — any SDK that sees it exports straight to the cloud, silently bypassing the Collector (no error, and the data still arrives). The standard name is reserved for "app → Collector"
  - **Traces**: the full `api` → Celery → `worker` chain is stitched together (verified: the same `trace_id` appears across both processes), with SQLAlchemy / Redis covered automatically. SDK init hangs off `worker_process_init` because `BatchSpanProcessor` is a background thread and **threads are not inherited across fork** — the same root cause, and the same hook, as `_dispose_inherited_engine`
  - **Logs**: structlog injects `trace_id` / `span_id` (W3C 32/16-digit hex). **Not routed over OTLP**: the logs pillar is the last to stabilise in Python, and cross-pillar correlation only needs those two fields
  - **Metrics (operational)**: `orders.raw.result`, `orders.processing.duration`, `orders.retry`, `circuit_breaker.state`, `recovery_scan.dispatched`; plus `http.server.duration` (P50/95/99) and `db.client.connections.usage` (pool pressure) supplied free by the instrumentation. The series budget is controlled with SDK Views — measured at **320 active series (3.2% of the 10k free tier)**. ⚠️ The counter-intuitive finding: **the expensive metrics are the automatic ones, not the custom ones**. The three Drop views remove `http.server.request/response.size` and `flower.task.runtime.seconds`, which together accounted for 27% — and the latter measured a second-scale value with millisecond-scale buckets, i.e. data with zero resolution
  - Exporter: Grafana Cloud (`ap-southeast-1`, Tempo + Prometheus)
- [—] OpenTelemetry — **three things deliberately not done** (`[—]` = decided against, each with a trigger; not a backlog)
  - **Business / DQ metrics**: `seed_demo_daily` picks its dirty rate deterministically from five choices per day → **constant within a day**, so a minute-level error rate says nothing `rpt_quality_events_daily` does not already say; high-cardinality slicing belongs in the warehouse by definition (see the tier-1/tier-2 boundary in [DQ_ARCHITECTURE.md](./DQ_ARCHITECTURE.md)). **Trigger**: a real upstream, where the dirty rate moves within a day
  - **Absent alerting and dashboards**: a liveness rule's threshold comes from the **schedule declaration**, not from a traffic distribution, so it is writable today — the reason not to write it is not capability but that **this is a portfolio project with no real traffic**: value thresholds (latency, error rate) and response procedures both need real traffic to grow, and the rules live in Grafana Cloud's UI where they can be neither version-controlled nor reviewed. **The reasoning is the deliverable** — six principles for setting them are recorded in [ORCHESTRATION §4.1](./ORCHESTRATION.md), including one counter-intuitive result: under cumulative temporality the series never goes absent, so `absent()` catches a dead process rather than a stopped upstream. **Trigger**: the day this system is actually used to monitor itself
  - **Airflow integration**: the Collector has been ready all along (plaintext to `otel-collector:4318`, auth added by the Collector, both already on the same compose network) — so **what blocks it is not technical**. It is not wired up because every consumer of it has been decided against: its strongest justification was supplying the analytics pipeline's liveness signal, and no rule will be written; its next was dashboard material, and dashboards are deprioritised; what remains is diagnostic value (splitting extract's time between PG and BQ), which is low for a daily batch with two samples a day — tracing's differentiating value is untangling concurrency, and extract is a four-step straight line in a single process. **Trigger**: writing liveness rules, or extract / dbt developing a real latency problem
