...
- [ ] dbt Core: rpt_*; includes `rpt_quality_*` (see [DQ_ARCHITECTURE.md](./DQ_ARCHITECTURE.md)). Scenario-specific `int_orders_*` and SCD2 `dim_customer` are designed but each enabled only when its trigger fires
- [ ] Looker Studio connected to BigQuery dim_*/fct_*/rpt_*
