...
**The data in this system is generated by `seed_demo.py`.** Its status distribution, its arrival pattern, and the ratio of `pending` to terminal rows are all artifacts of the generator's parameters. An index tuned against that distribution encodes the generator, not a workload.
...
The system's only data source is `seed_demo.py`, driven on a schedule by `seed_demo_daily`. Being explicit about its shape matters, because several deferrals above trace directly to it.
...
- **Bursts and backpressure.** Load characteristics come from `load_test.py` on demand, not from the scheduled generator.
