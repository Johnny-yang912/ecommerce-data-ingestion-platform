...
考慮過兩種形狀——**遷移形**（新列、同 `raw_id`，透過既有去重競爭）與**補丁形**（第二張修正表）。遷移形重用已經存在的機制；補丁形需要一份手工維護的第二宣告，並帶著一個重抽的地雷。
...
| 6 | quality_events | 對修正後的值重跑 `business_clean` 並 append 一筆帶 batch id 的事件，否則 Row Filter 的有效狀態合成會對不起來 |
